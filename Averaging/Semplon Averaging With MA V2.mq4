...
double  point              = 0;
double  buyMultiple        = 0;
double  sellMultiple       = 0;
...
int start(){
   ...
   if(ReachProfit == true && totalOpenOrder()>=1){
    ...
    //STOP LOSS
    if(AccountEquity()<=AB-(AB/100*MaxDailyStopLoss)){
       closeAll();
       ReachProfit = true;
       Position=POSITION_HOLD;
       lowestOrTopest     = 0;
    ...
    //tentukan posisi
     ...
     buyMultiple = 1;
     //tentukan signal
     if(space_buy<=space_with_ma*pnt && space_buy>0){
         buyMultiple = multiple;
     }

     sellMultiple = 1;
     if(space_sell<=space_with_ma*pnt && space_sell>0){
         sellMultiple = multiple;
     }
   ...
   if (
       ...
       if (Order==SIGNAL_BUY)
       {
          lastOpen = iTime(Symbol(),0,0);
          HLposition = Bid;
          OrderSend(Symbol(),OP_BUY, NormalizeDouble(optimizeLots()*buyMultiple, 2), Ask, Slippage, 0, Ask+InitialTakeProfit*pnt, "BELI", 10, 0, Green);
       }
       else if (Order==SIGNAL_SELL)
       {
         lastOpen = iTime(Symbol(),0,0);
         HLposition = Bid;
         OrderSend(Symbol(),OP_SELL, NormalizeDouble(optimizeLots()*sellMultiple, 2), Bid, Slippage, 0, Bid-InitialTakeProfit*pnt, "JUAL", 10, 0, Red);
       }
   }
